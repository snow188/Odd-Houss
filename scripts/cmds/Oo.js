...
if(event.senderID == global.game[event.threadID].player1.id || event.senderID == global.game[event.threadID].player2.id ){
  if(event.senderID == global.game[event.threadID].player1.id){
    ...
    message.reply({body:`Waouh! ${global.game[event.threadID].player2.name}🌮🫔\n Donc notre gagnant(e) est ${global.game[event.threadID].player1.name}.`, mentions: [{
                        tag: global.game[event.threadID].player1.name,
                        id: global.game[event.threadID].player1.id,
 ...
 message.reply("You don’t have any game running in this group")
      ...
      if(mention.length == 0) return message.reply("Mentionne le nom de ton adversaire 🥷⚔️🥷");
...
global.game[event.threadID] = {
  ...
  counting: 0
    ...
    }else{message.reply("Désolé mais un jeu est déjà en cours .......⛷️⛷️")}
...
message.send(global.game[event.threadID].board)
    ...
    if(global.game[event.threadID].turn === global.game[event.threadID].player1.id){
      setTimeout(function(){message.send({body:`Mes félicitations  ${global.game[event.threadID].player1.name} , 𝒕𝒖 𝒗𝒊𝒆𝒏𝒔 𝒅𝒆 𝐆𝐀𝐆𝐍𝐄𝐑 ..`, mentions: [{
                        tag: global.game[event.threadID].player1.name,
                        id: global.game[event.threadID].player1.id,
    ...
    }, 1000)} else {setTimeout(function(){message.send({body:`Félicitation  ${global.game[event.threadID].player2.name} , 𝒕𝒖 𝒗𝒊𝒆𝒏𝒔 𝒅𝒆 𝐆𝐀𝐆𝐍𝐄𝐑 ..`, mentions: [{
                        tag: global.game[event.threadID].player2.name,
                        id: global.game[event.threadID].player2.id,
  ...
  setTimeout(function (){message.send("Waouh vous êtes doué pour le jeu 🎮...")}, 1000)
  global.game[event.threadID].on = false
} else{
...
} else{message.reply("Désolé celui-ci est déjà bloqué 🚫")}

} else{message.reply("reply from 1-9")}
} else{message.reply("C'est pas ton tour mec ") }
